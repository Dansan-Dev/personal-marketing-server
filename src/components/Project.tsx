      ...
      <main className="project-component page-container">
        <article className="bg-[rgba(12,20,32,0.5)] border border-[var(--border-dark)] rounded-2xl shadow-md p-7 md:p-9">
          ...
          <header className="mb-6 md:mb-8">
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-[var(--text-light)]">{name}</h1>
          ...
          <section aria-labelledby="tech-title" className="mt-6 md:mt-8">
            <h2 id="tech-title" className="text-sm font-semibold uppercase tracking-wide text-[var(--text-dim)]">Tech Stack</h2>
            <div className="mt-3 flex flex-wrap gap-2.5">
              {tech.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-2 rounded-xl border border-[var(--border-dark)] bg-[rgba(255,255,255,0.06)] px-3 py-1.5 text-sm text-[var(--text-light)]"
                >
                  <span className="inline-block h-2 w-2 rounded-full bg-[var(--color-main)]" aria-hidden />
                  {t}
                </span>
              ))}
            </div>
          ...
          {highlights && highlights.length > 0 && (
            <section aria-labelledby="highlights-title" className="mt-8">
              ...
              <ul className="mt-3 grid gap-2 list-disc pl-6 text-[var(--text-light)]">
                {highlights.map((h, i) => (
                  <li key={i}>{h}</li>
                ))}
              </ul>
          ...
          {reflections && (
            <section aria-labelledby="reflections-title" className="mt-8">
              ...
              <p className="mt-3 text-[var(--text-light)] leading-relaxed whitespace-pre-line">{reflections}</p>
            </section>
          )}
